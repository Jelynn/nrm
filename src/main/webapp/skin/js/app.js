...
$(function () {
    ...
    var home = {
        ...
        bind: function () {
                ...
                //cm.api('./data/nrm.json').then(function(res){
                       ...
                       WeixinJSBridge.invoke('getBrandWCPayRequest', {
                           ...
                           function(s){
                               if(s.err_msg == "get_brand_wcpay_request:ok" ) {
                                    window.location.href = cm.url+window.location.search+'&orderId='+res.data.orderId+"#/payreturn";
                               }else{
                                    alert('支付失败，稍后再试！');
                               }
                           }
        ...
        setTimeout(function(){
            ...
            function order_list(res){
                ...
                $.each(res, function(k, v){
                    d_ += '<a href="javascript:void(0);" class="weui_media_box weui_media_appmsg"><div class="weui_media_hd"><img class="weui_media_appmsg_thumb" src="./skin/images/menu/'+v.img+'" alt=""></div><div class="weui_media_bd"><h4 class="weui_media_title">'+v.dishName+'</h4><p class="weui_media_desc">￥'+v.orderPrice+' <var>x'+v.quantity+'</var></p></div></a>'
                });
                return d_;
            }
            ...
            $('.content').dropload({
                ...
                loadDownFn : function(me){
                    cm.api(cm.url+'wxitf/cust_orderList',data).then(function(res){
                        counter++;
                        pageEnd = num * counter;
                        ...
                        $.each(d_, function(k, v){
                            order_view += '<div class="weui_panel weui_panel_access"><div class="weui_panel_hd"><span>订单号：'+v.orderId+'</span><span>总价:'+v.orderPrice+'</span></div><div class="weui_panel_bd">'+order_list(v.orderRelDtoList)+'</div></div>';
                            if(d_.length<10){
                                me.lock();
                                me.noData();
                            }
    ...
    var payreturn = {
        ...
        className: 'payreturn',
        render: function () {
            return $('#payreturn').html();
        },
        bind:function(){
            cm.api(cm.url+'order/getOrderInfo',{orderId:cm.geturl('orderId')}).then(function(res){
                $('#order_num').html(res.data.waitNo+'号');
            });
        }
    };
